...
sentinel_cfg = BPGCoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_bpgcoind_proto_version_with_sentinel_ping = 70207


def get_bpgcoin_conf():
    home = os.environ.get('HOME')

    bpgcoin_conf = os.path.join(home, ".bpgcoin/bpgcoin.conf")
    if sys.platform == 'darwin':
        bpgcoin_conf = os.path.join(home, "Library/Application Support/BPGCoin/bpgcoin.conf")

    bpgcoin_conf = sentinel_cfg.get('bpgcoin_conf', bpgcoin_conf)
