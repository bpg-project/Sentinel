...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, bpgcoind):
        golist = bpgcoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_bpgcoind(bpgcoind, item)
    ...
    @classmethod
    def import_gobject_from_bpgcoind(self, bpgcoind, rec):
        import decimal
        import bpgcoinlib
        import inflection
        ...
        # shim/bpgcoind conversion
        object_hex = bpgcoinlib.SHIM_deserialise_from_bpgcoind(object_hex)
        objects = bpgcoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from bpgcoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from bpgcoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from bpgcoind - BPGCoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(bpgcoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from bpgcoind! %s" % e)
            govobj.vote_delete(bpgcoind)
            return (govobj, None)
    ...
    def vote_delete(self, bpgcoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(bpgcoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, bpgcoind, signal, outcome):
        import bpgcoinlib
        ...
        output = bpgcoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = bpgcoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(bpgcoind, signal)

    def sync_network_vote(self, bpgcoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = bpgcoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 bpgcoin addr, non-multisig
            if not bpgcoinlib.is_valid_bpgcoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid BPGCoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (bpgcoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/BPGCoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import bpgcoinlib
        obj_data = bpgcoinlib.SHIM_serialise_for_bpgcoind(self.serialise())
    ...
    def prepare(self, bpgcoind):
        try:
            object_hash = bpgcoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not bpgcoinlib.is_valid_bpgcoin_address(addr, config.network):
    ...
    def hash(self):
        import bpgcoinlib
        return bpgcoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, bpgcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - bpgcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, bpgcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - bpgcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, bpgcoind):
        now = int(time.time())
        return (self.created_at < (now - bpgcoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, bpgcoind):
        if self.is_expired(bpgcoind):
            return False

        return True

    def is_deletable(self, bpgcoind):
        if self.is_expired(bpgcoind):
            return True
