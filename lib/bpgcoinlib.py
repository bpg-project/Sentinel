...
def is_valid_bpgcoin_address(address, network='mainnet'):
    ...
    bpgcoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid BPGCoin address
        return False

    if (address_version != bpgcoin_version):
        return False
...
# shims 'til we can fix the bpgcoind side
def SHIM_serialise_for_bpgcoind(sentinel_hex):
    ...
    # superblock => "trigger" in bpgcoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # bpgcoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    bpgcoind_hex = serialise(obj)
    return bpgcoind_hex


# shims 'til we can fix the bpgcoind side
def SHIM_deserialise_from_bpgcoind(bpgcoind_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(bpgcoind_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('bpgcoin.conf')
