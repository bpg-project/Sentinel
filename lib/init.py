...
def has_bpgcoin_conf():
    import config
    import io

    valid_bpgcoin_conf = False

    # ensure bpgcoin_conf exists & readable
    #
    # if not, print a message stating that BPGCoin Core must be installed and
    # configured, including JSONRPC access in bpgcoin.conf
    try:
        f = io.open(config.bpgcoin_conf)
        valid_bpgcoin_conf = True
    except IOError as e:
        print(e)

    return valid_bpgcoin_conf
...
def main():
    ...
    if not has_bpgcoin_conf():
        print("BPGCoin must be installed and configured, including JSONRPC access in bpgcoin.conf")
        sys.exit(1)
